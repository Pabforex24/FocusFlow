...
export default function TasksPage() {
  ...
  const handleSaveRecurring = (data: Omit<RecurringTemplate, 'id' | 'createdAt'>) => {
  ...
  return (
    <div className="px-4 pt-4 md:p-8 max-w-2xl mx-auto page-enter pb-24 md:pb-8 mt-[calc(env(safe-area-inset-top)+52px)] md:mt-0">
      ...
      >
        {([['tasks', 'Tâches', null], ['recurring', 'Récurrentes', recurringTemplates.filter(t => t.active).length]] as const).map(([id, label, count]) => (
          <button
            key={id}
            onClick={() => setTab(id)}
