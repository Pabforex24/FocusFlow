...
import { Domain, Goal, Task, RecurringTemplate, FrequencyType } from '@/types'
...
interface TaskModalProps {
  ...
  onSaveRecurring?: (data: Omit<RecurringTemplate, 'id' | 'createdAt'>) => void
  domains:  Domain[]
  goals:    Goal[]
  defaultDate?: Date
  existing?: Task | null
    ...
    <Modal open={open} onClose={onClose} title={existing ? 'Modifier la tâche' : 'Nouvelle tâche'}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Priorité">
          <select className={selectCls} value={priority} onChange={(e) => setPriority(e.target.value as typeof priority)}>
        ...
        >
          <Field label="Fréquence">
            <select
              className={selectCls}
              value={frequency}
              onChange={(e) => { setFrequency(e.target.value as FrequencyType); setCustomDays([]) }}
            >
