...
export function useNotifications() {
  ...
  return {
    ...
    setReminderMinutes: (m: number) => lsSet(KEY_REMINDER_MINUTES, String(m)),
    getStreakAlert:     () => lsGet(KEY_STREAK_ALERT) !== 'false',
    setStreakAlert:     (v: boolean) => lsSet(KEY_STREAK_ALERT, String(v)),
    getStreakHour:      () => parseInt(lsGet(KEY_STREAK_HOUR) || '20', 10),
    setStreakHour:      (h: number) => lsSet(KEY_STREAK_HOUR, String(h)),
